...
def save_local_key(aes_key, aes_iv, rsa_key, local_key_filename):
    ...
    cipher_bytes = pkcs1.encrypt(local_key_base64_bytes)
    with open(local_key_filename, 'wb') as fo:
        fo.write(cipher_bytes)


def load_local_key(rsa_key, local_key_filename):
    pkcs1 = PKCS1_OAEP.new(rsa_key)
    with open(local_key_filename, 'rb') as fo:
        cipher_bytes = fo.read()
    local_key_base64_bytes = pkcs1.decrypt(cipher_bytes)
...
def run_encryption_mode():
    ...
    src_filenames = ['sample.jpg', 'sample.txt']
    ...
    save_local_key(aes_key, aes_iv, rsa_public_key, default_local_key_filename)
...
def run_decryption_mode():
    ...
    rsa_private_key = input_rsa_private_key()
    aes_key, aes_iv = load_local_key(rsa_private_key, default_local_key_filename)
    print('AES Key:', aes_key)
